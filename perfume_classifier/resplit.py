...
def run_resplit(train_ratio: float = 0.8, seed: int = 42) -> None:
    ...
    current_total = 0
    for csv_path in [cfg.path.train_csv, cfg.path.val_csv, cfg.path.test_csv]:
        if os.path.exists(csv_path):
            current_total += len(pd.read_csv(csv_path))
    if current_total and len(df) < current_total * 0.5:
        raise RuntimeError(
            f"Refusing to resplit: source CSV has {len(df)} rows, but current "
            f"data/All splits have {current_total} rows. The added-data source "
            "is not data/raw/all_cleaned.csv in this checkout. Restore the full "
            "source CSV or resplit from the correct added-data file."
        )
