...
원본 데이터에는 총 242개 브랜드가 존재하지만, 이 중 15개 이상 샘플을 가진 브랜드는 35개이다. 따라서 본 보고서의 Brand 분류 결과는 전체 242개 브랜드 전체에 대한 성능이 아니라, 학습과 평가가 가능한 35개 주요 브랜드를 대상으로 한 closed-set 분류 성능으로 해석한다.
...
현재 정확도는 35개 주요 브랜드에 대한 분류 정확도의 근거로 사용할 수 있다. 그 이유는 다음과 같다.

| 근거 | 내용 |
|------|------|
| 독립된 Test set 사용 | train/validation/test를 80/10/10으로 분리하고, 최종 정확도는 학습에 사용하지 않은 test 데이터에서 계산하였다. |
| Stratified split 적용 | 각 브랜드가 train/validation/test에 비슷한 비율로 나뉘도록 분할하여 특정 브랜드만 과도하게 test에 들어가는 문제를 줄였다. |
| 낮은 기준선 대비 높은 성능 | 35개 브랜드의 랜덤 기준선은 약 2.86%, test 최빈 클래스 기준선은 약 4.88%인데, 모델은 95.12%를 기록하였다. |
| 특정 다수 브랜드 의존 가능성 낮음 | test 데이터는 브랜드당 1~4개 수준으로 분산되어 있어, 상위 몇 개 브랜드만 많이 맞혀서 나온 점수로 보기 어렵다. |
| 시각적 단서가 태스크와 직접 연결 | 브랜드 로고, 병 형태, 라벨 디자인은 이미지 안에 직접 나타나므로 이미지 기반 모델이 학습할 수 있는 정보가 충분하다. |
| Val/Test 결과 차이가 작음 | 기록된 validation accuracy는 97.53%, test accuracy는 95.12%로 큰 차이가 없어, 35개 브랜드 내부에서는 과적합 신호가 강하지 않다. |

따라서 95.12%는 "35개 주요 브랜드 중 하나로 분류하는 문제에서는 모델이 높은 정확도를 보인다"는 주장에 대한 근거가 된다. 즉, 현재 실험 조건 안에서는 Brand 분류가 이미지 기반 모델에 적합하다는 결론을 뒷받침한다.

다만 이 정확도를 전체 브랜드 분류 성능으로 일반화해서는 안 된다. 원본 데이터의 전체 브랜드는 242개이고, 현재 평가는 그중 35개 브랜드만 사용하였다. 샘플 수가 부족한 207개 브랜드는 평가에서 제외되었기 때문에, 현재 결과는 "전체 242개 브랜드를 모두 맞히는 성능"이나 "처음 보는 브랜드까지 분류하는 성능"의 근거는 아니다.

정리하면, 현재 Brand 정확도는 제한된 조건에서는 신뢰할 수 있는 분류 정확도 근거이지만, 그 범위는 `35개 주요 브랜드 closed-set 분류`로 한정된다. 전체 브랜드 대상으로 일반화하려면 샘플 수 기준을 낮춘 추가 실험이나, 희소 브랜드를 `Other/Unknown`으로 묶는 별도 평가가 필요하다.
...
본 프로젝트에서는 향수병 이미지를 활용한 Brand 분류와 Note 분류를 수행하였다. Brand 분류는 EfficientNet-B0를 통해 95.12%의 높은 정확도를 달성하였다. 이는 브랜드가 병 디자인과 강하게 연결되어 있기 때문이다. 이 정확도는 35개 주요 브랜드 closed-set 분류 성능의 근거로는 사용할 수 있지만, 전체 242개 브랜드 전체에 대한 일반화 성능으로 해석해서는 안 된다.
